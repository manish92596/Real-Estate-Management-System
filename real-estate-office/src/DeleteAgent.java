...
public class DeleteAgent extends JFrame {
   ...
   public static void DeleteAndAssign(String del_id, String old_id) {
      ...
      try {
         ...
         SQL = "Update manages set A_id='" + old_id + "' where A_id='" + del_id + "'";

         st.executeUpdate(SQL);
         // stmt.setInt(2, delId);
         // stmt.setInt(1, oldId);
         // stmt.executeQuery();
         System.out.println("manages update done");

         // SQL = "Update contact set AGEN_id=? where AGEN_id=? ";
         String SQL1 = "Update contact set agen_id='" + old_id + "' where agen_id='" + del_id + "'";

         st.executeUpdate(SQL1);
         System.out.println("contact update done");
         ...
         add_del_agent = "insert into Deleteagent"
               + "(DA_id,DA_name,DA_password,DA_email,DA_address,DA_username,DA_contact_no)" + "VALUES(' " + A_ID
               + " ','" + A_name + "','" + A_password + "','" + A_email + "','" + A_address + "','" + A_username + "',"
               + A_contact_str + ")";

         st.executeUpdate(add_del_agent);
         System.out.println("deleted agent delete table me chala gya");

         String deal = "Update deals set a_id='" + old_id + "' where a_id='" + del_id + "'";
         st.executeUpdate(deal);
         System.out.println("deal update");
         ...
         public void actionPerformed(ActionEvent e) {
            ...
            if (inputText.isEmpty() || inputText1.isEmpty()) {
               ...
               if (agent_exists(inputText) == 1) {
                  ...
                  JOptionPane.showMessageDialog(null, "Deleted Successfully", "Message",
