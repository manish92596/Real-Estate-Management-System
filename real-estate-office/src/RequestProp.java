...
public class RequestProp extends JFrame {
    ...
    static int count = 0;
    static int T_id = 0;
    static final String url = "jdbc:mysql://localhost:3306/real_estate_office";
    ...
    public static void update(String propertyid, int price, String Date, int B_id) {
        try {
            ...
            Statement stmt = conn.createStatement();
            ResultSet resultSet;
            String query2 = "select manages.A_id from manages where P_id='" + propertyid + "'";
            ...
            String SQL = "insert into transaction" + "(P_id,T_id,SR_price,SR_date,B_id)" + "VALUES('" + propertyid
                    + "','" + T_id + "','" + price + "','" + Date + "','" + B_id + "')";

            // String SQL1 = "insert into manages" + "(P_id,A_id)" + "VALUES('" + propertyid
            // + "','" + A_id + "')";
    ...
    public RequestProp() {
        ...
        try {
            ...
            // Create a statement and execute the SQL query
            Statement stmt = conn.createStatement();
            String query = " select buyer.B_id,B_name,B_email,B_contact_no,B_address,deals.p_id,status from buyer,deals,property where a_id='"
                    + v1 + "' and status='Available' and buyer.b_id=deals.b_id and deals.p_id=property.p_id;";

            ResultSet rs = stmt.executeQuery(query);

            // Get the metadata of the result set to determine the number and names of
            // columns
            ...
            // Create the JTable with the retrieved data and column names, and add it to a
            // scroll pane
        ...
        try {
            ...
            submit.addActionListener(new ActionListener() {
                public void actionPerformed(ActionEvent e) {
                    ...
                    // frame.dispose();
                    if (input.isEmpty() || Dateinput.isEmpty()) {
                        JOptionPane.showMessageDialog(null, "Please enter Price and Date", "Warning",
                                JOptionPane.WARNING_MESSAGE);

                    } else {
                        if (selectedP_id == null) {
                            JOptionPane.showMessageDialog(null, "Please Select a Property to continue", "Warning",
                                    JOptionPane.WARNING_MESSAGE);
                        } else {
                            update(selectedP_id, Priceinput, Dateinput, selectedB_id);
                            updateprop(selectedP_id);
                            JOptionPane.showMessageDialog(null, "Property Sold with transaction id: " + T_id,
