...
public class BuyerSingup extends JFrame {
	...
	public static void update(String propertyid, int B_id) {
		try {
			...
			ResultSet resultSet;
			String query2 = "select A_id from manages where P_id='" + propertyid + "'";
			resultSet = stmt.executeQuery(query2);
			resultSet.next();
			int A_id = resultSet.getInt(1);
			System.out.println(A_id);

			String SQL = "insert into deals" + "(A_id,B_id,P_id)" + "VALUES('" + A_id + "','" + B_id + "','"
					+ propertyid + "')";
	...
	public static void insertnewbuyer(String name, String password, String email, String address, String username,
			String contactno) {
		...
		try {
			...
			SQL = "insert into buyer" + "(B_id,B_name,B_password,B_email,B_address,B_username,B_contact_no)"
	...
	public BuyerSingup() {
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (name.isEmpty() || password.isEmpty() || email.isEmpty() || address.isEmpty() || username.isEmpty()
						|| contact.isEmpty()) {
